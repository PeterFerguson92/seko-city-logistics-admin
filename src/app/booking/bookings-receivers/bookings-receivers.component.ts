...
export class BookingsReceiversComponent implements OnInit {
    ...
    ngOnInit(): void {
        ...
        this.destinationForm = this.formBuilder.group({
            destination: [this.destination ? this.destination : this.destinations[0], [Validators.required]],
            location: [''],
        });
    ...
    isDisabled() {
        const isDisabled =
            this.receiversCustomerForm.valid === false ||
            this.getDestinationFormControl('location').valid === false;

        if (isDisabled) {
            this.alertService.success(
                'Ops, Some informations are still missing, can you please complete all required informations',
                this.alertOptions
            );
        }
        return isDisabled;
    ...
    getReceiversFormControl(fControlName: string, index) {
        ...
        return fControlName === 'phone' || fControlName === 'countryCode'
            ? fControl.get('phoneGroup').get(fControlName)
    ...
    validateReceiverFormControl(fControlName: string, index) {
        const fControl = this.getReceiversFormControl(fControlName, index);
        this.validationService.watchAndValidateFormControl(fControl).subscribe(() => {
            this.formValidationMapList[index][fControlName] = this.validationService.getValidationMessage(
                fControl,
                fControlName
            );
        });
    ...
    validateGroupFormControl(formGroupName: string, fControlName: string, index) {
        ...
        this.validationService.watchAndValidateFormControl(fGroup).subscribe(() => {
            this.formValidationMapList[index].phone = this.validationService.getGroupValidationMessage(
                fGroup,
                fMainControl,
                fControlName
            );
    ...
    getDestinationInfo() {
        return {
            destination: this.getDestinationFormControl('destination').value,
            location: this.getDestinationFormControl('location').value,
        };
    }
}
