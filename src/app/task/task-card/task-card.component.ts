...
export class TaskCardComponent implements OnInit {
    ...
    getCreationDateFormatted(creationDate) {
        return new Date(creationDate * 1000).toUTCString();
    }
    ...
    getTaskClass(task) {
        ...
        const taskDueDate = new Date(
            this.commonService.getFormattedDate(task.actionDate).split('/').reverse().join('/')
        );

        if (todayNewDate > taskDueDate && task.status !== 'DONE') {
            return { pastDueDate: true };
        }

        if (
            this.commonService.getFormattedDate(today) === this.commonService.getFormattedDate(task.actionDate) &&
            task.status !== 'DONE'
        ) {
